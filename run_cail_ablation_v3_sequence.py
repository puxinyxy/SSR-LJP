...
OUTPUT_DIR = ROOT / "embedding_output" / "ablation_v3"
REPORT_PATH = OUTPUT_DIR / "CAIL2018_RAG_ablation_results.md"
STATE_PATH = OUTPUT_DIR / "sequence_state.json"
LOG_PATH = OUTPUT_DIR / "sequence_monitor.log"
CURRENT_EXPERIMENT_PID = 24388
...
def common_law_args(output_name: str) -> list[str]:
    return [
        "eval_llm_rag_law.py",
        "--dataset_path",
        ...
        "2000",
        "--output_path",
        f"embedding_output\\ablation_v3\\{output_name}",
    ]


def common_penalty_args(output_name: str) -> list[str]:
    return [
...
EXPERIMENTS = [
    {
        ...
        "name": "法条预测 - Embedding",
        "task": "law",
        "output": "cail_law_embedding.jsonl",
        "args": common_law_args("cail_law_embedding.jsonl")
        + ["--retrieval-mode", "embedding", "--no-rerank"],
        "attach_pid": CURRENT_EXPERIMENT_PID,
    },
    {
        "id": "law_hybrid_rrf",
        "name": "法条预测 - Hybrid RRF",
        "task": "law",
        "output": "cail_law_hybrid_rrf.jsonl",
        "args": common_law_args("cail_law_hybrid_rrf.jsonl")
        + HYBRID_ARGS
        + ["--no-rerank"],
    },
    {
        ...
        "task": "law",
        "output": "cail_law_hybrid_rerank.jsonl",
        "args": common_law_args("cail_law_hybrid_rerank.jsonl")
        + HYBRID_ARGS
    ...
    },
    {
        ...
        "name": "刑期预测 - Embedding",
        "task": "penalty",
        "output": "cail_penalty_embedding.jsonl",
        "args": common_penalty_args("cail_penalty_embedding.jsonl")
        + ["--retrieval-mode", "embedding", "--no-rerank"],
    },
    {
        ...
        "task": "penalty",
        "output": "cail_penalty_hybrid_rrf.jsonl",
        "args": common_penalty_args("cail_penalty_hybrid_rrf.jsonl")
        + HYBRID_ARGS
        + ["--no-rerank"],
    },
    {
        ...
        "task": "penalty",
        "output": "cail_penalty_hybrid_rerank.jsonl",
        "args": common_penalty_args("cail_penalty_hybrid_rerank.jsonl")
        + HYBRID_ARGS
        + ["--use-rerank", "--rerank-top-k", "30"],
    },
]
...
def calculate_metrics(exp: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    ok, output = run_metric_command(
        ["--input", relative_output, "--mode", exp["task"]]
    )
    if not ok:
        raise RuntimeError(f"最终指标计算失败：\n{output}")
...
def render_report(state: dict[str, Any]) -> None:
    lines = [
        f"# {DATASET_NAME} RAG 消融实验结果",
        "",
        f"- 本轮开始时间：{state['sequence_started_at']}",
        f"- 后台监控心跳：{state['heartbeat_at']}",
        f"- 总体状态：{state['status']}",
        f"- 样本数：{EXPECTED_SAMPLES}",
        "- 执行顺序：法条预测 3 组，刑期预测 3 组",
        "",
        "## 执行状态",
        "",
        "| 序号 | 实验 | 状态 | 进度 | 开始时间 | 完成时间 |",
        "|---:|---|---|---:|---|---|",
    ]
    ...
    for index, exp in enumerate(EXPERIMENTS, start=1):
        ...
        lines.append(
            ...
            f"{item['progress']}/{EXPECTED_SAMPLES} | "
    ...
    lines.extend(
        [
            ...
            "## 法条预测结果",
            "",
            "| 实验 | Accuracy | Macro-P | Macro-R | Macro-F1 |",
            "|---|---:|---:|---:|---:|",
        ]
    )
    lines.extend(law_rows or ["| 暂无已完成实验 | - | - | - | - |"])
    ...
    lines.extend(penalty_rows or ["| 暂无已完成实验 | - | - | - | - |"])
    ...
    for exp in EXPERIMENTS:
        ...
        lines.extend(
            [
                f"### {item['name']}",
                "",
                "| K | Any Hit | All Hit | Recall | MRR | 排名来源 |",
                "|---:|---:|---:|---:|---:|---|",
            ]
        )
    ...
    if not retrieval_found:
        lines.append("暂无已完成实验。")

    lines.extend(["", "## Penalty Precedent Retrieval Metrics", ""])
    penalty_retrieval_found = False
    for exp in EXPERIMENTS:
        if exp["task"] != "penalty":
            continue
        item = state["experiments"][exp["id"]]
        ...
        lines.append("")
    if not penalty_retrieval_found:
        lines.append("No completed penalty retrieval metrics.")
...
def update_progress(
    state: dict[str, Any], exp: dict[str, Any], status: dict[str, Any]
) -> None:
    item = state["experiments"][exp["id"]]
...
def wait_for_completion(
    state: dict[str, Any], exp: dict[str, Any], pid: int
) -> dict[str, Any]:
    ...
    while True:
        status = inspect_jsonl(output_path)
...
def main() -> int:
    ...
    save_state(state)
    render_report(state)
    log(f"monitor started pid={os.getpid()} attached_experiment_pid={CURRENT_EXPERIMENT_PID}")

    for index, exp in enumerate(EXPERIMENTS):
        item = state["experiments"][exp["id"]]

        existing_status = inspect_jsonl(OUTPUT_DIR / exp["output"])
        if existing_status["complete"]:
            log(
                f"reusing completed output {exp['id']} "
                f"records={existing_status['records']}"
            )
            try:
        ...
        if exp.get("attach_pid"):
            pid = int(exp["attach_pid"])
            log(f"attaching to existing experiment {exp['id']} pid={pid}")
        else:
